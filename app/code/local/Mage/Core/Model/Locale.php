...
class Mage_Core_Model_Locale
{
    ...
    //Cmsmart Add
    public function checkVerMagento(){
        return substr(Mage::getVersion(), 0, 3);
    }
    public function getTranslatedOptionLocales1()
    {
        return $this->_getOptionLocales1(true);
    }


    protected function _getOptionLocales1($translatedName=false)
    {
        if ($this->checkVerMagento() == '1.9') {
            $options = array();
            $zendLocales = $this->getLocale()->getLocaleList();
            $languages = $this->getLocale()->getTranslationList('language', $this->getLocale());
            $countries = $this->getCountryTranslationList();
